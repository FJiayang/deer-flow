...
- [🦌 DeerFlow - 2.0](#-deerflow---20)
  ...
  - [クイックスタート](#クイックスタート)
    ...
    - [アプリケーションの実行](#アプリケーションの実行)
      - [オプション1: ビルド済みイメージ（推奨）](#オプション1-ビルド済みイメージ推奨)
      - [オプション2: Docker開発（ホットリロード）](#オプション2-docker開発ホットリロード)
      - [オプション3: ローカル開発](#オプション3-ローカル開発)
    - [詳細設定](#詳細設定)
...
最速の起動方法 — ローカルビルド不要。GitHub Container Registryのマルチアーキテクチャイメージ（amd64/arm64）を使用します。

```bash
# 1. 設定ファイルを生成
make config

# 2. config.yaml を編集 — 少なくとも1つのモデルとAPIキーを設定
# 3. .env を編集 — APIキーを設定（OPENAI_API_KEY、TAVILY_API_KEY など）
...
#### オプション2: Docker開発（ホットリロード）

ソースマウントによるライブ開発のためにローカルでイメージをビルド：
