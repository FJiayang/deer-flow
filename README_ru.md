...
- [🦌 DeerFlow - 2.0](#-deerflow---20)
  ...
  - [Быстрый старт](#быстрый-старт)
    ...
    - [Запуск](#запуск)
      - [Вариант 1: Готовые образы (рекомендуется)](#вариант-1-готовые-образы-рекомендуется)
      - [Вариант 2: Docker-разработка (hot-reload)](#вариант-2-docker-разработка-hot-reload)
      - [Вариант 3: Локальная разработка](#вариант-3-локальная-разработка)
...
#### Вариант 1: Готовые образы (рекомендуется)

Самый быстрый способ запуска — без локальной сборки. Используются мультиархитектурные образы (amd64/arm64) из GitHub Container Registry.

```bash
# 1. Сгенерировать файлы конфигурации
make config

# 2. Отредактировать config.yaml — настроить хотя бы одну модель и API-ключ
# 3. Отредактировать .env — указать API-ключи (OPENAI_API_KEY, TAVILY_API_KEY и др.)
...
Указать конкретную версию вместо `latest`:
...
#### Вариант 2: Docker-разработка (hot-reload)

Локальная сборка образов с монтированием исходников для разработки в реальном времени:
